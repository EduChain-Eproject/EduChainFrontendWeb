...
import { fetchBlog } from '../redux/blogActions';
...
import { Sidebar } from '../../../../front-end/blogs/presentation/components/Sidebar';
import {
  createBlogComment,
  deleteBlogComment,
  fetchBlogsComment,
  updateBlogComment,
} from '../../../blogComment/presentation/redux/BlogCommentActions';
import { CreateBlogCommentReq } from '../../../blogComment/domain/usecases/CreateBlogComment';
import { UpdateBlogCommentReq } from '../../../blogComment/domain/usecases/UpdateBlogComment';

export const BlogDetailPageComp: React.FC = () => {
  const dispatch = useAppDispatch();
  const { data, status, error } = useAppSelector((s) => s.blogs.blog);
  const { blogId } = useParams<{ blogId: string }>();
  const [refreshComments, setRefreshComments] = useState(false);
  const [commentText, setCommentText] = useState<string>('');
  const [replyText, setReplyText] = useState<string>('');
  const [activeComment, setActiveComment] = useState<number | null>(null);
  const createComment = useAppSelector((s) => s.commnent.createComment);
  const userId = useAppSelector((s) => s.auth.user?.id);
  const fetchComments = useAppSelector((s) => s.commnent.fetchComments);

  useEffect(() => {
    const id = blogId ? Number(blogId) : Number(localStorage.getItem('blogId'));
    if (id) {
      dispatch(fetchBlogsComment(id));
  ...
  const handleCommentSubmit = () => {
    ...
    dispatch(createBlogComment(commentData))
      .unwrap()
      .then((response) => {
  ...
  const handleEditSubmit = () => {
    ...
    const req: UpdateBlogCommentReq = {
      text: editCommentText,
    };

    dispatch(updateBlogComment({ id, req }))
      .unwrap()
      .then((response) => {
        console.log('Comment submitted successfully:', response);
        setCommentText('');
  ...
  const handleDeleteComment = (commentId: number) => {
    ...
    dispatch(deleteBlogComment(commentId))
      .unwrap()
      .then(() => {
        setRefreshComments((prev) => !prev);
      });
            ...
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  {/* <p className="text-sm text-gray-500">Category: {data?.blogCategoryId}</p> */}
                </div>
              </div>
            </div>
            <h2 className="text-xl font-bold mb-4">{data?.title}</h2>
            {/* <img
                            src={`${import.meta.env.VITE_API_BASE_URL}/uploads/${data?.photo}`}
                            alt={data?.title}
...
export default BlogDetailPageComp;
