...
export const fetchBlogs = createAsyncThunk('blogs/fetchBlogs', async () => {
    ...
    return await getBlogsUseCase.execute();
});
...
export const createBlog = createAsyncThunk('blogs/createBlog', async (blogData: CreateBlogReq) => {
    const createBlogUseCase = new CreateBlog(blogRepository);
    return await createBlogUseCase.execute(blogData);
});

export const updateBlog = createAsyncThunk('blogs/updateBlogs', async ({ blogId, blogData }: { blogId: number, blogData: any }) => {
