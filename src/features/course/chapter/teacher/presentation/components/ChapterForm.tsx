// File: ../components/ChapterForm.tsx
...
const ChapterForm: React.FC<ChapterFormProps> = ({ courseId }) => {
    ...
    const { handleSubmit, control, formState: { errors } } = useForm<CreateChapterReq>({
        defaultValues: {
            chapterTitle: '',
        }
    });
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <input
                            {...field}
                            type="text"
                            className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 ${errors.chapterTitle ? 'border-red-500' : ''}`}
                        />
                    )}
                />
                {errors.chapterTitle && <p className="text-red-500 text-sm mt-1">{errors.chapterTitle.message}</p>}
            </div>
