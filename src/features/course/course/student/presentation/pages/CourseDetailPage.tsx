...
const CourseDetailPage: React.FC = () => {
    ...
    return (
        <div className="p-4">
            ...
            <div className="mb-4">
                <button onClick={() => setActiveTab('detail')} className={`px-4 py-2 rounded-md ${activeTab === 'detail' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>Detail</button>
                <button onClick={() => setActiveTab('curriculum')} className={`px-4 py-2 rounded-md ${activeTab === 'curriculum' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>Curriculum</button>
                <button onClick={() => setActiveTab('teacher')} className={`px-4 py-2 rounded-md ${activeTab === 'teacher' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>Teacher</button>
                <button onClick={() => setActiveTab('feedback')} className={`px-4 py-2 rounded-md ${activeTab === 'feedback' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>Feedback</button>
            </div>
            {activeTab === 'detail' && courseData && <CourseDetail course={courseData} />}
            {activeTab === 'curriculum' && courseData && <Curriculum chapters={courseData.chapters} onLessonClick={handleLessonClick} />}
            {activeTab === 'teacher' && courseData &&
                <Teacher
