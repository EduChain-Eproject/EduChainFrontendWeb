...
const CourseFormUpdate: React.FC<CourseFormUpdateProps> = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 p-6 bg-white shadow-lg rounded-lg max-w-4xl mx-auto">
      {error && (
      ...
      <div className="flex flex-col lg:flex-row lg:space-x-4">
        <div className="flex-1">
          ...
          <input
            id="title"
            {...register('title')}
            className="mt-1 p-4 bg-blue-100 text-gray-700 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
          />
          {errors?.title && (
  ...
  className="mt-1 p-3 bg-blue-100 text-gray-700 border border-gray-300 rounded-md block w-full focus:ring-2 focus:ring-blue-300 focus:border-blue-300"
/>
          {errors?.description && (
            ...
            <div className="py-1" role="none">
              <div className="grid grid-cols-5 gap-4 px-4 py-2">
                {listCategories?.map((cate) => (
                  <div
                    key={cate.id}
                    className="flex items-center p-2 rounded-md hover:bg-blue-100 hover:shadow-md hover:border-blue-500 transition-all"
                  >
                    <Controller
      ...
      <div className="flex justify-center mt-6">
        <button
          type="submit"
          className="justify-center items-center rounded-lg border border-transparent bg-blue-600 px-8 py-4 text-base font-semibold text-white shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-300 ease-in-out transform w-80 hover:scale-105"
        >
          Submit
        </button>
      </div>
